...
export default function AdsPerformanceTable({ ads }: AdsPerformanceTableProps) {
    ...
    <section className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-12">
      <div className="p-6 border-b border-gray-100 bg-slate-50/50 flex justify-between items-center">
        ...
        <div className="relative">
          ...
          >
            <span>📊 Columns</span>
            <svg className={`w-4 h-4 transition-transform ${showColumnSelector ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="Link9 5l7 7-7 7"></path></svg>
          </button>
            ...
            {ads.map((ad) => (
              <tr key={ad.id} className="hover:bg-gray-50 transition-colors text-sm">
                ...
                {visibleColumns.has("campaign") && (
                  <td className="px-6 py-4 font-medium text-gray-700 whitespace-nowrap">{ad.campaign_name}</td>
                )}
                ...
                {visibleColumns.has("adGroup") && (
                  <td className="px-6 py-4 text-gray-600 whitespace-nowrap">{ad.group_name}</td>
                )}
