...
interface PublisherAlertSectionProps {
  publisherId: number;
  initialAlerts: {
...
const STORAGE_KEY = 'publisherAlertExpanded';

export default function PublisherAlertSection({
  publisherId,
  initialAlerts,
}: PublisherAlertSectionProps) {
  ...
  const handleDismiss = useCallback(async (alertId: string) => {
    ...
    try {
      const response = await fetch('/api/publisher/alerts', {
        method: 'POST',
      ...
      {activeAlerts.length > 0 && (
        ...
        >
          <span className="text-xs font-bold text-gray-700">
            💡 {activeAlerts.length}件のお知らせがあります
          </span>
          <svg
