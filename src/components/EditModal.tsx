            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" name="start_date" defaultValue={data.start_date?.split(' ')[0]} className="w-full p-2 border rounded-lg text-sm" required />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-1">End Date</label>
                  <input type="date" name="end_date" defaultValue={data.end_date?.split(' ')[0]} className="w-full p-2 border rounded-lg text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Budget (¥)</label>
                <input type="number" name="budget" defaultValue={data.budget} className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-slate-200" required />
              </div>
            </>
          )}
          ...
          {type === "adGroup" && (
            <>
              ...
              <div>
                ...
                <input type="number" name="max_bid" defaultValue={data.max_bid} className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-emerald-100" required />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Target Device</label>
                <div className="flex gap-4">
                  {['all', 'desktop', 'mobile'].map(d => (
                    <label key={d} className="inline-flex items-center text-sm font-medium">
          ...
          {type === "ad" && (
            <>
              ...
              <div>
                ...
                <input type="text" name="title" defaultValue={data.title} className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-blue-100" required />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Description</label>
                <textarea name="description" defaultValue={data.description} className="w-full p-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-100" rows={2} />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Image URL</label>
                <input type="url" name="image_url" defaultValue={data.image_url} className="w-full p-2 border rounded-lg text-xs" required />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Target URL</label>
                <input type="url" name="target_url" defaultValue={data.target_url} className="w-full p-2 border rounded-lg text-xs" required />
              </div>
            </>
          )}
